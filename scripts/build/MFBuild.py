...
def run_ant_file(build_file, source_dir, load_dir, ant_home, full_build, set64bit):
    ...
    if os_type == 'Linux':
        ...
        ant_cmd = ['sh', ant_exe, '-lib', mfant_jar, '-f', build_file, '-Dbasedir', source_dir, '-Dloaddir', load_dir, '-D64bitset', set64bit]
        useShell = False
    else:
        ant_cmd = [ant_exe, '-lib', mfant_jar, '-f', build_file, '-Dbasedir', source_dir, '-Dloaddir', load_dir, '-D64bitset', set64bit]
        useShell = True
    #write_log(ant_cmd)
