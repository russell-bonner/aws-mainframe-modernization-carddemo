...
def create_new_system(template_base, region_location, sys_base):

    parentdir = str(Path(sys_base).parents[0])
    isExist = os.path.exists(region_location)
    if isExist == False:
        os.mkdir(region_location)
...
def deploy_system_modules (repo_dir, sys_base, os_type, is64bit, database_type):
    ...
    target_load = os.path.join(sys_base, 'sysloadlib')
...
def deploy_vsam_data (repo_dir, sys_base, os_type, esuid):
    ...
    source_load = os.path.join(repo_dir, 'datafiles', 'vsam')
...
def dbfhdeploy_sysfile (sysfile, os_type, is64Bit, mfdbfh_location):
    ...
    if os_type == 'Windows':
	    ...
	    bin = 'bin'
    dbfhdeploy = os.path.join(os.environ['COBDIR'], bin, 'dbfhdeploy')
    catalog_location = mfdbfh_location.format(sysfile)
    dbfhdeploy_cmd = '\"{}\" add \"{}\" \"{}\"'.format(dbfhdeploy, sysfile, catalog_location)
    subprocess.run([dbfhdeploy, "add", sysfile, catalog_location])


def deploy_application (repo_dir, sys_base, os_type, is64bit):
    ...
    source_load = os.path.join(repo_dir, 'loadlib')
    copy_tree(source_load, target_load)
